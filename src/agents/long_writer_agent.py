...
def init_long_writer_agent(config: LLMConfig) -> Agent:
    return Agent(
        name="Long Writer Agent",
        model=config.model,
        description="An expert report writer tasked with iteratively writing each section of a report",
...
async def write_next_section(
    ...
    result = await long_writer_agent.run(
        user_message,
    )
